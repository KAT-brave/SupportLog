...
ポートフォリオとしては、単なるCRUDアプリではなく、実際の業務運用を想定した、ステータス変更履歴、担当者管理、ユーザー承認制、論理削除などを意識して実装しています。
...
※以下のような業務上重要な処理についてMinitestを使用してテストを追加しています。
...
また、GitHub Actionsでテストと静的解析を実行し、変更によって既存機能が壊れていないか確認しています。
...
- 担当者は自分が担当している問い合わせのみ編集・削除可能
- 担当者以外の一般ユーザーは編集・削除不可
